...
| Architecture design | 1–4 | Design a minimal but meaningful agent workflow that uses RevenueCat. Example: an AI writing assistant agent that checks entitlements before generating content, uses RC to gate premium features, and reports usage via the Charts API. Stack: Python (FastAPI) agent backend, RC SDK for entitlement checks, simple CLI or web frontend. |
