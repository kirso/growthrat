...
const articles: Article[] = [
  ...
  },
  {
    ...
    description:
      "Portfolio experiment brief showing how GrowthRat would measure data-grounded content against a control once activated.",
    category: "experiment",
    pubDate: "2026-03-15",
  },
  {
    ...
    description:
      "GrowthRat's first weekly report: content produced, experiment brief drafted, feedback submitted, and lessons learned.",
    category: "report",
    pubDate: "2026-03-16",
  },
];
...
function getSectionArticles(allArticles: Article[], key: string) {
  return allArticles.filter((a) => a.category === key);
}

export default async function ProofPackPage() {
  // Merge hardcoded seed articles with dynamically published articles from Convex
  let allArticles = [...articles];
  const convexUrl = process.env.NEXT_PUBLIC_CONVEX_URL;
  if (convexUrl) {
    try {
      const convex = new ConvexHttpClient(convexUrl);
      const published = await convex.query(api.artifacts.listPublished, {});
      if (published && published.length > 0) {
        const dynamicArticles: Article[] = published
          .filter((a: { slug: string }) => !articles.some((s) => s.slug === a.slug)) // Dedupe against seed
          .map((a: { slug: string; title: string; content: string; artifactType: string; publishedAt?: number; _creationTime: number }) => ({
            slug: a.slug,
            title: a.title,
            description: a.content.slice(0, 150).replace(/[#*_`]/g, "") + "...",
            category: a.artifactType === "blog_post" ? "technical" : a.artifactType,
            pubDate: a.publishedAt
              ? new Date(a.publishedAt).toISOString().split("T")[0]
              : new Date(a._creationTime).toISOString().split("T")[0],
          }));
        allArticles = [...articles, ...dynamicArticles];
      }
    } catch {
      // Convex unavailable — use seed articles only
    }
  }

  const sectionsWithCounts = sections.map((s) => ({
    ...s,
    items: getSectionArticles(allArticles, s.key),
  }));
